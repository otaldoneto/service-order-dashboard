...
import { ApiError, ApiUnavailableError, apiRequest, buildUrl } from "@/lib/api/client";
...
describe("buildUrl", () => {
  ...
  it("adds the query parameters and leaves out the empty ones", () => {
    const url = buildUrl("/orders", { status: "OPEN", page: 0, title: "", clientId: undefined });
...
describe("apiRequest", () => {
  ...
  it("returns the parsed JSON body", async () => {
    fetchMock.mockResolvedValue(jsonResponse({ email: "demo@helpdesk.dev", roles: ["VIEWER"] }));
  ...
  it("sends the token as a Bearer header and the body as JSON", async () => {
    ...
    await apiRequest("/auth/login", { method: "POST", token: "abc", body: { email: "a@b.com" } });
  ...
  it("turns an error response into an ApiError with the API's status and message", async () => {
    fetchMock.mockResolvedValue(
      jsonResponse({ status: 403, error: "Forbidden", message: "Access denied" }, 403),
    );

    const error = await apiRequest("/clients", { method: "POST" }).catch((e: unknown) => e);
  ...
  it("still returns an ApiError when the error body is not JSON", async () => {
    ...
    await expect(apiRequest("/orders")).rejects.toMatchObject({ status: 502, message: "HTTP 502" });
  });

  it("turns a network failure or a timeout into an ApiUnavailableError", async () => {
    fetchMock.mockRejectedValue(new TypeError("fetch failed"));

    await expect(apiRequest("/orders")).rejects.toBeInstanceOf(ApiUnavailableError);
  });

  it("reports a missing API_BASE_URL as a configuration error, not as an unavailable API", async () => {
    vi.stubEnv("API_BASE_URL", "");

    await expect(apiRequest("/orders")).rejects.toThrow("API_BASE_URL is not set");
